// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using Test_MVC.Data;

#nullable disable

namespace Test_MVC.Migrations
{
    [DbContext(typeof(CampDbContext))]
    [Migration("20220623091655_eventsCanBeNull")]
    partial class eventsCanBeNull
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.6")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("EventParticipantVolunteerPerson", b =>
                {
                    b.Property<int>("EventsId")
                        .HasColumnType("int");

                    b.Property<int>("ParticipantsId")
                        .HasColumnType("int");

                    b.HasKey("EventsId", "ParticipantsId");

                    b.HasIndex("ParticipantsId");

                    b.ToTable("EventParticipantVolunteerPerson");
                });

            modelBuilder.Entity("ShowTrainer", b =>
                {
                    b.Property<int>("ShowsId")
                        .HasColumnType("int");

                    b.Property<int>("TrainersId")
                        .HasColumnType("int");

                    b.HasKey("ShowsId", "TrainersId");

                    b.HasIndex("TrainersId");

                    b.ToTable("ShowTrainer");
                });

            modelBuilder.Entity("Test_MVC.Models.Camera", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Company")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Model")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Camera");
                });

            modelBuilder.Entity("Test_MVC.Models.CameraUsage", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("CameraId")
                        .HasColumnType("int");

                    b.Property<int>("CameraOperatorId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("CameraId");

                    b.HasIndex("CameraOperatorId");

                    b.ToTable("CameraUsage");
                });

            modelBuilder.Entity("Test_MVC.Models.Event", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int>("Status")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Event");

                    b.HasDiscriminator<string>("Discriminator").HasValue("Event");
                });

            modelBuilder.Entity("Test_MVC.Models.Event+Show", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<double>("Duration")
                        .HasColumnType("float");

                    b.HasKey("Id");

                    b.ToTable("Show");
                });

            modelBuilder.Entity("Test_MVC.Models.Person", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<double?>("AmountOfHoursWorked")
                        .HasColumnType("float");

                    b.Property<DateTime?>("ArrivalDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("ContactDetails")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<DateTime?>("DepartureDate")
                        .HasColumnType("datetime2");

                    b.Property<string>("Discriminator")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("DormitoryNumber")
                        .HasColumnType("int");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<int?>("RoomNumber")
                        .HasColumnType("int");

                    b.Property<string>("Surname")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Person");

                    b.HasDiscriminator<string>("Discriminator").HasValue("Person");
                });

            modelBuilder.Entity("Test_MVC.Models.Product", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhotoURL")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<double>("Price")
                        .HasColumnType("float");

                    b.Property<int?>("ShopId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("ShopId");

                    b.ToTable("Product");
                });

            modelBuilder.Entity("Test_MVC.Models.Room", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Area")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Room");
                });

            modelBuilder.Entity("Test_MVC.Models.Shop", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Shop");
                });

            modelBuilder.Entity("Test_MVC.Models.Sponsor", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("Id");

                    b.ToTable("Sponsor");
                });

            modelBuilder.Entity("Test_MVC.Models.SponsorOpenEvent", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<float>("AmountReceivedFromSponsors")
                        .HasColumnType("real");

                    b.Property<int>("OpenEventId")
                        .HasColumnType("int");

                    b.Property<int>("SponsorId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("OpenEventId");

                    b.HasIndex("SponsorId");

                    b.ToTable("SponsorOpenEvents");
                });

            modelBuilder.Entity("TrainerTraining", b =>
                {
                    b.Property<int>("TrainersId")
                        .HasColumnType("int");

                    b.Property<int>("TrainingsId")
                        .HasColumnType("int");

                    b.HasKey("TrainersId", "TrainingsId");

                    b.HasIndex("TrainingsId");

                    b.ToTable("TrainerTraining");
                });

            modelBuilder.Entity("Test_MVC.Models.Close_Event", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Event");

                    b.HasDiscriminator().HasValue("Close_Event");
                });

            modelBuilder.Entity("Test_MVC.Models.Open_Event", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Event");

                    b.Property<int>("MaxAmountOfPeople")
                        .HasColumnType("int");

                    b.Property<string>("StreetAddress")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)");

                    b.HasDiscriminator().HasValue("Open_Event");
                });

            modelBuilder.Entity("Test_MVC.Models.Operator", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Person");

                    b.Property<int>("LicenceNumber")
                        .HasColumnType("int");

                    b.HasDiscriminator().HasValue("Operator");
                });

            modelBuilder.Entity("Test_MVC.Models.Organiser", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Person");

                    b.Property<int>("WorkSector")
                        .HasColumnType("int");

                    b.HasDiscriminator().HasValue("Organiser");
                });

            modelBuilder.Entity("Test_MVC.Models.ParticipantVolunteerPerson", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Person");

                    b.HasDiscriminator().HasValue("ParticipantVolunteerPerson");
                });

            modelBuilder.Entity("Test_MVC.Models.Trainer", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Person");

                    b.HasDiscriminator().HasValue("Trainer");
                });

            modelBuilder.Entity("Test_MVC.Models.Training", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Event");

                    b.Property<DateTime>("EndDate")
                        .HasColumnType("datetime2");

                    b.Property<int>("RoomId")
                        .HasColumnType("int");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime2");

                    b.HasIndex("RoomId");

                    b.HasDiscriminator().HasValue("Training");
                });

            modelBuilder.Entity("Test_MVC.Models.CameraOperator", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Operator");

                    b.HasDiscriminator().HasValue("CameraOperator");
                });

            modelBuilder.Entity("Test_MVC.Models.MusicConsoleOperator", b =>
                {
                    b.HasBaseType("Test_MVC.Models.Operator");

                    b.Property<bool>("OwnEquipment")
                        .HasColumnType("bit");

                    b.HasDiscriminator().HasValue("MusicConsoleOperator");
                });

            modelBuilder.Entity("EventParticipantVolunteerPerson", b =>
                {
                    b.HasOne("Test_MVC.Models.Event", null)
                        .WithMany()
                        .HasForeignKey("EventsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Test_MVC.Models.ParticipantVolunteerPerson", null)
                        .WithMany()
                        .HasForeignKey("ParticipantsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("ShowTrainer", b =>
                {
                    b.HasOne("Test_MVC.Models.Event+Show", null)
                        .WithMany()
                        .HasForeignKey("ShowsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Test_MVC.Models.Trainer", null)
                        .WithMany()
                        .HasForeignKey("TrainersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Test_MVC.Models.CameraUsage", b =>
                {
                    b.HasOne("Test_MVC.Models.Camera", "Camera")
                        .WithMany("CameraUsages")
                        .HasForeignKey("CameraId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Test_MVC.Models.CameraOperator", "CameraOperator")
                        .WithMany("CameraUsages")
                        .HasForeignKey("CameraOperatorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Camera");

                    b.Navigation("CameraOperator");
                });

            modelBuilder.Entity("Test_MVC.Models.Product", b =>
                {
                    b.HasOne("Test_MVC.Models.Shop", "Shop")
                        .WithMany("Products")
                        .HasForeignKey("ShopId");

                    b.Navigation("Shop");
                });

            modelBuilder.Entity("Test_MVC.Models.SponsorOpenEvent", b =>
                {
                    b.HasOne("Test_MVC.Models.Open_Event", "OpenEvent")
                        .WithMany("SponsorOpenEvents")
                        .HasForeignKey("OpenEventId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Test_MVC.Models.Sponsor", "Sponsor")
                        .WithMany("SponsorOpenEvents")
                        .HasForeignKey("SponsorId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("OpenEvent");

                    b.Navigation("Sponsor");
                });

            modelBuilder.Entity("TrainerTraining", b =>
                {
                    b.HasOne("Test_MVC.Models.Trainer", null)
                        .WithMany()
                        .HasForeignKey("TrainersId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("Test_MVC.Models.Training", null)
                        .WithMany()
                        .HasForeignKey("TrainingsId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Test_MVC.Models.Training", b =>
                {
                    b.HasOne("Test_MVC.Models.Room", "Room")
                        .WithMany("Trainings")
                        .HasForeignKey("RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("Room");
                });

            modelBuilder.Entity("Test_MVC.Models.Camera", b =>
                {
                    b.Navigation("CameraUsages");
                });

            modelBuilder.Entity("Test_MVC.Models.Room", b =>
                {
                    b.Navigation("Trainings");
                });

            modelBuilder.Entity("Test_MVC.Models.Shop", b =>
                {
                    b.Navigation("Products");
                });

            modelBuilder.Entity("Test_MVC.Models.Sponsor", b =>
                {
                    b.Navigation("SponsorOpenEvents");
                });

            modelBuilder.Entity("Test_MVC.Models.Open_Event", b =>
                {
                    b.Navigation("SponsorOpenEvents");
                });

            modelBuilder.Entity("Test_MVC.Models.CameraOperator", b =>
                {
                    b.Navigation("CameraUsages");
                });
#pragma warning restore 612, 618
        }
    }
}
